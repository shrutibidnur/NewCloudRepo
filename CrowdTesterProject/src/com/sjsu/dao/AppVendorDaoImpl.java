...
public class AppVendorDaoImpl implements IAppVendorDao{
	...
	public List<BugDetailsBO> viewBugsForAppProvider(
		...
		criteria.add(Restrictions.eq("appDetails.applicationID",testApplicationDetails.getApplicationID()).ignoreCase());
...
//			Set Ranking based on credit
			if (testerDetailsToEdit.getCredit() >= 300) {
				testerDetailsToEdit.setRanking(1);
			} else if (testerDetailsToEdit.getCredit() >= 250) {
				testerDetailsToEdit.setRanking(2);
			} else if(testerDetailsToEdit.getCredit() >= 200) {
